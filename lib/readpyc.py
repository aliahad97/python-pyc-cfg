...
import typing
import uncompyle6
if (sys.version_info[0] == 3 and sys.version_info[1] >= 7): import decompyle3
...
def join_two_dict_of_lists(x , y):
    '''
    Merges two dictionaries together

    params: x: dict[str, list[types.CodeType]], y: dict[str, list[types.CodeType]]
    return: dict[str, list[types.CodeType]]
    '''
    return { key:x.get(key,[])+y.get(key,[]) for key in set(list(x.keys())+list(y.keys())) }

def recur_co_map(co):
    '''
    Returns a dictionary of function name mapped to code objects

    params: co: types.CodeType
    return: dict[str, list[types.CodeType]]
    '''
    dict_mapping = {}
    fn_name= co.co_name
    dict_mapping[fn_name] = [co]
    for const in co.co_consts:
        if isinstance(const, types.CodeType):
            new_map = recur_co_map(const)
            dict_mapping = join_two_dict_of_lists(dict_mapping, new_map)
    return dict_mapping

def co_map_from_file(filename):
    '''
    Returns a dictionary of function name mapped to code objects
    ...
    '''
    try:
        co_curr = read_file_get_object(filename)
        return recur_co_map(co_curr)
    except:
        return {}
